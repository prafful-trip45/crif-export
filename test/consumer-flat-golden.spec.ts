...
// converter must reproduce exactly. The header in the provided file is itself
// off-spec, so we assert the body + a valid 146-byte header separately.
const EXPECTED_REC0 =
  'Dhaval Navnitkumar Patel200619782AIDPP2356F982582222526/B, Keshavbag Society, Sabarmati, Ramnagar, Ahmedabad243800050101NB94430001ADARSHCAPDNP0105111062020150520265000000500000000000109';
const EXPECTED_REC1 =
  'Upendra Chinubhai Shah300119582AIOPS3732M98250051533, Nandi Hill Satellite Road Opp. I.S.R.O Ahmedabad City Ambawadi Vistar Ahmedabad Gujarat243800150101NB94430001ADARSHCAPUCS010510710202515052026110000001100000000000109';
...
maybe('Consumer flat (Data Submission Form) — real pair', () => {
  it('reproduces each consumer record byte-for-byte from the real input', async () => {
    ...
    // Line 0 = 146-byte TUDF header + record0; line 1 = record1.
    const header = lines[0]!.slice(0, 146);
    const rec0 = lines[0]!.slice(146);
    expect(header.length).toBe(146);
    expect(header.startsWith('TUDF12NB94430001')).toBe(true);
